...
Asena.addCommand({ pattern: 'insta ?(.*)', fromMe: false, desc: IG_DESC }, async (message, match) => {
    ...
    if (type === 'image') { await message.sendMessage(data, MessageType.image, { caption: "⬇️Downloaded by Sachusamu" }) }
    else if (type === 'video') { await message.sendMessage(data, MessageType.video, { caption: "⬇️Downloaded by Sachusamu" }) }
});
//const axios = require('axios')
